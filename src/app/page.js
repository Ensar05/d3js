...
export default function Home() {
  ...
  const [filteredCategory, setFilteredCategory] = useState(["allgemein"]);
                  ...
                  {searchedButtons
                    ...
                    .map((button) => (
                      <button
                        key={button.name}
                        value={button.name}
                        onClick={() => svgNodes.createSVG(d3.select('#svg-container'), `${button.name}`, `${button.color}`, darkMode,`${button.connection}`)}
